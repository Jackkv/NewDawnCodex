# Character Creation

Creating the perfect character in New Dawn requires understanding stats, skills, and templates. This guide will help you make informed decisions.

## Character Basics

### Stat Allocation

You start with **80 stat points** to distribute among three attributes:

| Stat | Abbreviation | Affects | Recommended Starting Range |
|------|--------------|---------|----------------------------|
| Strength | STR | Hit Points, Melee Damage, Carrying Capacity | 45-60 |
| Dexterity | DEX | Stamina, Swing Speed, Defense | 10-25 |
| Intelligence | INT | Mana, Magic Effectiveness | 10-25 |

!!! tip "Stat Caps"
    - Individual stat cap: 100
    - Total stat cap: 225 (you can raise stats beyond starting values)

### Skill Selection

You have **700 total skill points** that can be distributed across many skills:

- Skills range from 0.0 to 100.0
- Choose 3-5 primary skills to focus on initially
- Skills increase through use

## Popular Templates

### Warrior Templates

#### Swordsman
Perfect for new players who enjoy melee combat.

**Skills:**

- Swordsmanship: 100
- Tactics: 100
- Anatomy: 100
- Healing: 100
- Parrying: 80
- Resisting Spells: 80
- Magery: 40 (for utility)

**Stats:** STR 100 / DEX 80 / INT 45

#### Archer
Ranged combat specialist.

**Skills:**

- Archery: 100
- Tactics: 100
- Anatomy: 100
- Healing: 100
- Hiding: 80
- Resisting Spells: 80
- Magery: 40

**Stats:** STR 80 / DEX 100 / INT 45

### Mage Templates

#### Pure Mage
Masters of magic and offensive spells.

**Skills:**

- Magery: 100
- Evaluating Intelligence: 100
- Meditation: 100
- Resisting Spells: 100
- Wrestling: 80
- Inscription: 80
- Poisoning: 40

**Stats:** STR 45 / DEX 60 / INT 120

#### Battle Mage
Combines magic with combat prowess.

**Skills:**

- Magery: 100
- Evaluating Intelligence: 100
- Swordsmanship: 100
- Tactics: 100
- Anatomy: 100
- Resisting Spells: 100
- Meditation: 100

**Stats:** STR 80 / DEX 60 / INT 85

### Crafting Templates

#### Blacksmith
Create weapons and armor.

**Skills:**

- Blacksmithy: 100
- Mining: 100
- Tinkering: 100
- Arms Lore: 100
- Magery: 80
- Meditation: 80
- Resisting Spells: 40

**Stats:** STR 100 / DEX 60 / INT 65

#### Tailor
Master of leather and cloth work.

**Skills:**

- Tailoring: 100
- Tinkering: 100
- Arms Lore: 100
- Magery: 80
- Meditation: 80
- Resisting Spells: 80
- Item Identification: 60

**Stats:** STR 70 / DEX 60 / INT 95

### Hybrid Templates

#### Tamer
Control powerful creatures to fight for you.

**Skills:**

- Animal Taming: 100
- Animal Lore: 100
- Veterinary: 100
- Magery: 100
- Evaluating Intelligence: 100
- Meditation: 100
- Resisting Spells: 100

**Stats:** STR 45 / DEX 50 / INT 130

## Customizing Your Appearance

### Character Appearance

When creating your character, you can customize:

- **Race:** Human or Elf (if available)
- **Gender:** Male or Female
- **Hair Style:** Multiple options
- **Hair Color:** Various colors
- **Facial Hair:** For male characters
- **Skin Tone:** Multiple skin tones

### Starting Equipment

Based on your skill selection, you'll receive:

- Basic clothing
- A few starting weapons/tools
- Some gold coins (usually 100-1000)
- Basic supplies

## Tips for Character Creation

!!! success "Best Practices"
    - **Start simple** - Choose a basic template for your first character
    - **Research skills** - Read our [Skills Guide](../game-mechanics/skills/index.md)
    - **Consider playstyle** - Pick skills that match how you want to play
    - **Leave room to grow** - Don't max all skills immediately
    - **Ask for advice** - The community can help on Discord

## Skill Growth Strategy

### Early Game (0-50 skill)

Skills gain quickly through basic use:
- Hunt weak creatures
- Practice on training dummies
- Craft basic items

### Mid Game (50-80 skill)

Gains slow down:
- Hunt moderate difficulty creatures
- Participate in group activities
- Focus on quality training

### Late Game (80-100 skill)

Very slow gains:
- Challenge difficult content
- Use skill gain bonuses if available
- Consider skill scrolls (if available)

## Respeccing

!!! info "Changing Your Build"
    If you want to change your skills later:
    
    1. Skills can be lowered/raised naturally
    2. Use skill locks (up/down/locked)
    3. Some servers offer skill balls or respec items
    
    Check with staff about respec options!

## Multiple Characters

Consider creating multiple characters for different roles:

- **Main Character** - Your primary character
- **Crafter** - For creating items
- **Resource Gatherer** - Mining, lumberjacking
- **Merchant** - For trading and house vendors

## Next Steps

Once you've created your character:

1. Complete the [New Player Guide](new-player-guide.md) tutorial
2. Learn the [Controls & Interface](controls-interface.md)
3. Study specific [Skills](../game-mechanics/skills/index.md)
4. Explore the [World](../world/index.md)

---

**May your character become a legend in Britannia!**
